...
//  pocket-tts-macos
//
//  Saved-voice catalog data models, extracted from VoiceManager.swift so the
//  pure engine layer (and the headless CLI/daemon target) can reference them
//  without pulling in the @MainActor manager and its AVFoundation import.
//  `VoiceManager` owns the behavior; these are just the persisted shapes.
//
